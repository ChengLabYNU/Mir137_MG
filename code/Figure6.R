...
write.csv(result, file="up_gene_tss_count.csv", quote=F,row.names=F)

up_result = melt(result,measure.vars = c('Pu.1_cKO','Pu.1_cWT','H3K27ac_cKO','H3K27ac_cWT','H3K4me3_cKO','H3K4me3_cWT'))
up_result$group = unlist(lapply(strsplit(as.character(up_result$variable),'_'),'[',1))
up_result$genotype = unlist(lapply(strsplit(as.character(up_result$variable),'_'),'[',2))
up_result$genotype = factor(up_result$genotype,levels = c('cWT','cKO'))
up_result$value = as.numeric(up_result$value)

pu1_up_plot = ggplot(filter(up_result,group == 'Pu.1'),aes(x=genotype,y=value,color=genotype))+
...
ggsave('Figure6J-1.pdf',pu1_up_plot,width = 3,height = 3.8)

H3K4me3_up_plot = ggplot(filter(up_result,group == 'H3K4me3'),aes(x=genotype,y=value,color=genotype))+
  geom_jitter(size=1,width =0)+
  geom_boxplot(fill="#FFFFFF")+
  scale_color_manual(values = c(cWT ='black',cKO='red'))+
  stat_compare_means(comparisons = list(c('cWT','cKO')),method = "t.test",
  ...
  theme(axis.text.y = element_text(color='black',size=8),
        ...
        plot.title = element_text(hjust = 0.5),
        legend.position = 'none')
ggsave('Figure6J-2.pdf',H3K4me3_up_plot,width = 2.8,height = 3.8)


H3K27ac_up_plot = ggplot(filter(up_result,group == 'H3K27ac'),aes(x=genotype,y=value,color=genotype))+
  geom_jitter(size=1,width =0)+
  geom_boxplot(fill="#FFFFFF")+
  scale_color_manual(values = c(cWT ='black',cKO='red'))+
  stat_compare_means(comparisons = list(c('cWT','cKO')),method = "t.test",
  ...
  theme(axis.text.y = element_text(color='black',size=8),
        ...
        plot.title = element_text(hjust = 0.5),
        legend.position = 'none')
ggsave('Figure6J-3.pdf',H3K27ac_up_plot,width = 2.8,height = 3.8)
